...
import { getLinks } from '../src/linkGenerator'

test('should generate simple link', async () => {
    const result = getLinks(
        "[A-Z][A-Z]+-\\d+",
        "https://example.com/browse/$0",
...
test('should generate link by subgroup match', async () => {
    const result = getLinks(
        ...
        ["some text with #123 issue in it"]
    )
...
test('should generate multiple links from single message', async () => {
    const result = getLinks(
        "#(\\d+)",
        "https://example.com/issues/$1",
        ["some text with #123 and #234 issues in it"]
    )
